...
```{mermaid}
flowchart LR
    subgraph Logical["逻辑 KV Cache 序列"]
        L0["Block 0: Token 0~15"]
        L1["Block 1: Token 16~31"]
        L2["Block 2: Token 32~47"]
    end
    subgraph PageTable["Page Table 逻辑/物理映射表"]
        M0["Block 0 ➔ 物理槽位 #8"]
        M1["Block 1 ➔ 物理槽位 #3"]
        M2["Block 2 ➔ 物理槽位 #12"]
    end
    subgraph Physical["非连续物理显存池 (GPU HBM)"]
        P3["Physical Block #3"]
        P8["Physical Block #8"]
        P12["Physical Block #12"]
        PFree["空闲 Block 池 (零内部碎片)"]
    end
    L0 --> M0 --> P8
    L1 --> M1 --> P3
    L2 --> M2 --> P12
