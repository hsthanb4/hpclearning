...
```{mermaid}
flowchart TD
    Prompt["输入提示词 (Prompt q)"] --> Sampling["Old Policy 采样 G 个不同回答 {o_1, o_2, ..., o_G}"]
    Sampling --> Reward["环境/规则计算奖励 {r_1, r_2, ..., r_G}"]
    Reward --> Stats["计算组内均值 μ 与标准差 σ"]
    Stats --> NormAdv["组内相对优势: A_i = (r_i - μ) / (σ + ε)"]
    NormAdv --> Loss["裁剪替代目标更新 Policy (无需 Critic 价值网络)"]
```
