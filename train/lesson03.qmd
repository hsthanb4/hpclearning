...
- **Broadcast**：一个 root 的数据复制给所有 rank（数据不改变）。
- **Reduce**：各 rank 的数据经二元运算（SUM 等）合并，结果只到 root。
- **AllReduce**：Reduce + 结果广播给所有 rank。DP 同步梯度就用它。

### 2. 分发与收集模式

- **Gather / AllGather**：各 rank 的**不同**数据块汇集到一个 root / 所有 rank。与 Broadcast 的区别：每个 rank 贡献的是自己独有的数据。
- **Scatter**：root 的数据按块分发给各 rank（Gather 的逆）。
- **ReduceScatter**：先按块 reduce，再把每块结果留在对应 rank 上——每个 rank 只得到**一部分**汇总结果。通信量约为 all-reduce 的一半。
- **Barrier**：同步点，所有 rank 到齐才放行。滥用会拖慢整体。

### 3. Ring AllReduce：把通信摊到邻居之间

朴素做法是每对 rank 直接通信（all-to-all），连接数和流量随 N 增长，不可扩展。Ring AllReduce 把 rank 排成环，分两个阶段，每步只与相邻 rank 通信：

1. **ReduceScatter 阶段**（N−1 步）：每个 rank 把数据切成 N 块，逐步把"自己积累的块"传给下一个 rank，同时累加从上一个 rank 收到的块。结束后每个 rank 持有**一块完整的汇总结果**（不同 rank 持有不同块）。
2. **AllGather 阶段**（N−1 步）：每个 rank 把自己那块汇总结果沿环转发，直到所有 rank 都拿到全部 N 块。

通信量：每个 rank 每步发送 \(K/N\) 个数值（K 为数据总量），共 \(2(N-1)\) 步，即

$$\text{每卡发送通信量} = 2 \cdot \frac{N-1}{N} \cdot K \approx 2K \quad (N \gg 1)$$

- AllReduce 总成本 ≈ \(2K\)；
- ReduceScatter 与 AllGather 各 ≈ \(K\)（这就是"reduce-scatter 比 all-reduce 快两倍"的准确含义，也是 ZeRO 省通信的数学基础）。

---

### 图解：AllReduce 与 ReduceScatter + AllGather 对比

<div align="center">
  <img src="assets/allreduce_vs_reducescatter_allgather.jpeg" width="90%" alt="AllReduce vs ReduceScatter + AllGather 对比图解" />
  <p><em>图 1：AllReduce vs ReduceScatter + AllGather 机制与显存/通信对比（本地 Gemini 绘图模型生成）</em></p>
</div>

<div align="center">
  <img src="assets/reducescatter_allgather.svg" width="90%" alt="分块状态流转图" />
  <p><em>图 2：Ring 算法各 Rank 分块在 ReduceScatter 与 AllGather 两个阶段的流转状态</em></p>
</div>
